...
def get_all_holidays(start_year, end_year):
    ...
    """
    # 1. Feriados do Banco (Locais e fixados manualmente)
    ...
    db_dates = {f['date'] for f in feriados_lista if not f['repeats_annually']}
    db_annual_days = {(f['date'].month, f['date'].day) for f in feriados_lista if f['repeats_annually']}
    ...
    if start_year and end_year:
        ...
        for dt, name in br_holidays.items():
            ...
            feriados_lista.append({
                ...
                'repeats_annually': False, # A library gerencia a data exata a cada ano (importante pra feriados moveis)
                'apply_to_all': True,
                'target_companies': [],
                'target_departments': [],
                'target_turnos': [],
