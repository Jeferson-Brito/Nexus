...
def verificacao_lojas(request):
    ...
    if cached_stats:
        total_active, irregular_count, ok_count, suspended_count, irregular_store_ids = (
            cached_stats['total_active'], cached_stats['irregular_count'], 
            cached_stats['ok_count'], cached_stats['suspended_count'], cached_stats['irregular_store_ids']
        )
    else:
        ...
        ok_count = base_stats_query.filter(audits__isnull=False).exclude(audit_issues__status='aberta').distinct().count()
        ...
        cache.set(cache_key, {
            'total_active': total_active, 'irregular_count': irregular_count, 'ok_count': ok_count,
            'suspended_count': suspended_count, 'irregular_store_ids': irregular_store_ids
        }, 300)
    ...
    elif tab == 'irregular' and request.user.role in ['gestor', 'administrador']:
    ...
    weekly_audits = StoreAudit.objects.filter(created_at__gte=start_week)
    if scope == 'my_stores': weekly_audits = weekly_audits.filter(analyst=request.user)
    weekly_audits_count = weekly_audits.count()
    
    weekly_irregular = StoreAuditItem.objects.filter(audit__created_at__gte=start_week, is_compliant=False)
    if scope == 'my_stores': weekly_irregular = weekly_irregular.filter(audit__analyst=request.user)
    
    weekly_total = StoreAuditItem.objects.filter(audit__created_at__gte=start_week)
    if scope == 'my_stores': weekly_total = weekly_total.filter(audit__analyst=request.user)
    
    total_items = weekly_total.count()
    compliance_rate = (weekly_total.filter(is_compliant=True).count() / total_items * 100) if total_items > 0 else 100
